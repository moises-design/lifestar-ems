...
const feats = [
  { Icon: FaClock,    title: 'On Time, Every Time',     desc: 'We track appointment schedules carefully so your child never misses therapy.' },
  { Icon: FaHeart,    title: 'Patient & Compassionate', desc: 'Our staff is trained to work with children — calm, kind, and always caring.' },
  { Icon: FaComments, title: 'Family Communication',    desc: 'We keep parents informed and coordinate directly with therapy providers.' },
  { Icon: FaShieldAlt,title: 'Safe Transport',          desc: 'Fully equipped, clean vehicles with child safety features.' },
]

const stars = Array.from({length:45},()=>({ left:`${Math.random()*100}%`, top:`${Math.random()*100}%`, size:`${Math.random()*2+1}px`, delay:`${Math.random()*4}s`, dur:`${2+Math.random()*3}s` }))
    ...
    <div className="sp therapy-sp">
      <section className="sp-hero therapy-hero">
        <div className="sp-hero-bg">
          ...
          {/* FA icon decoration */}
          <FaChild className="therapy-deco-icon" />
        </div>
        <div className="container sp-inner">
          <div className="sp-badge"><FaChild style={{marginRight:6}} /> Pediatric Therapy Transportation</div>
          ...
          <div className="sp-btns">
            <Link to="/request" className="btn btn-blue"><FaChild /> Request Therapy Transport</Link>
      ...
      <section className="sp-cta-banner">
        <div className="container">
          ...
          <Link to="/request" className="btn btn-blue btn-lg"><FaChild /> Request Therapy Transport</Link>
